...
//------------------------------------------------------------------------------
	...
	if ((dri_fd  = open(DRIFILE,O_RDWR | O_CLOEXEC)) == -1) {
		r = errno;
		log_error("open %s: %s(%d)\n", DRIFILE, strerror(r), r);
		goto finally;
	}
...
//------------------------------------------------------------------------------
	...
	if ((r = ioctl(dri_fd, DRM_IOCTL_SET_MASTER, 0)) != 0) {
		r = errno;
		log_error("DRM_IOCTL_SET_MASTER: %s(%d)\n", strerror(r), r);
		goto finally;
	}

	//Get resource counts
	if ((r = ioctl(dri_fd, DRM_IOCTL_MODE_GETRESOURCES, &res)) != 0) {
		r = errno;
