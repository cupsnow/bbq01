...
linux_MAKEPARAM = CROSS_COMPILE=$(CROSS_COMPILE) ARCH=arm LOADADDR=0x82000000
linux_MAKEPARAM += INSTALL_HDR_PATH=$(DESTDIR)/usr
linux_MAKEPARAM += -C $(linux_DIR)

linux_config:
	$(MAKE) $(linux_MAKEPARAM) multi_v7_defconfig
...
toolchain_libs_install:
	$(MKDIR) $(DESTDIR)/lib
	for i in libc.so.* libc-*.so libm.so.* libm-*.so; do \
	  $(CP) -d $(CROSS_COMPILE_PATH)/arm-none-linux-gnueabi/libc/lib/$$i $(DESTDIR)/lib; \
	done
...
dist2:
	...
	$(CP) $(linux_DIR)/arch/arm/boot/dts/am335x-bone.dtb $(dist_DIR)/boot/dtb
